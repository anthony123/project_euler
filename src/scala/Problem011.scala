...
// Finds the maximum product of 4 adjacent cells in the specified list.
def listMax(list: List[Int], max: Int = 0): Int =
  if(list.length < 4) max
  else listMax(list.tail, scala.math.max(max, list(0) * list(1) * list(2) * list(3)))

// Finds the maximum product of 4 adjacent horizontal cells in the specified grid.
def horizontalMax(grid: List[List[Int]], max: Int = 0) = {
  grid.foldLeft(0) {(a, b) => scala.math.max(a, listMax(b, max))}
}

// Finds the maximum product of 4 adjacent vertical cells in the specified grid.
def verticalMax(grid: List[List[Int]], max: Int = 0): Int =
  if(grid(0).length > 0) verticalMax(grid.map {_.tail}, scala.math.max(max, listMax(grid.map {_.head})))
  else max

def downwardDiagMax(grid: List[List[Int]]) = {
  var max = 0

  for(i <- 0 until grid.length - 4; j <- 0 until grid.length - 4) {
    max = scala.math.max(max, grid(i)(j) * grid(i + 1)(j + 1) * grid(i + 2)(j + 2) * grid(i + 3)(j + 3))
  }

  max
}

def upwardDiagMax(grid: List[List[Int]]) = {
  var max = 0
